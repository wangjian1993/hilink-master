<template>
 <div class="container">
   <div v-show="!isShowVideo" class="album" :style="{ backgroundImage:'url('+dataList.videoAlbum.picurl+')',backgroundSize:'100% 100%'}">
   </div>
   ...
   <div class="intro">
     <p style="font-size:16px;font-weight:bold">说明:</p>
     <p style="font-size:14px;margin-top:10px;color:#666666">{{dataList.videoAlbum.description}}</p>
   </div>
     ...
     <div class="cell" v-for="(item,index) in dataList.list" :key="index">
       ...
       <div class="cellRight" @click="goPlayVideo(item.picurl,item.videopath)">
         <div>
           ...
           <p style="color:#666666">{{item.timelong}}</p>
         </div>
         <div style="width:50px;height:22px;color:#AAAAAA;border:1px solid #AAAAAA;text-align:center;border-radius:5px;" >
          播放
         </div>
       </div>
     </div>
...
import http from '../../api/index.js'
 export default {
   ...
   async mounted(){
     this.isLoaded = true
     await http.getVideoList(this.$route.params.id).then(res=>{
        //转换播放时间
